...
class App extends Component {
  ...
  _createObstacle(lastLeft, index) {
    const leftDiff =
      Math.floor(Math.random() * OBSTACLE_MAX_DISTANCE) + OBSTACLE_MIN_DISTANCE;
    return {
  ...
  _onInterval() {
    ...
    this.setState((oldState) => {
      ...
      this._resetControls();
      let obstacleSpeed =
        INITIAL_OBSTACLE_STEP + INITIAL_OBSTACLE_STEP * 0.2 * level;
      if (right) {
      ...
      if (now - this._lastNewObstacles >= NEW_OBSTACLES_TIME) {
        ...
        if (numOfObstacles) {
          lastLeft =
            oldState.obstacles[numOfObstacles - 1].left - INITIAL_OBSTACLE_STEP;
        ...
        for (let i = 0; i < 2; i++) {
          const newObstacle = this._createObstacle(
            lastLeft,
            obstacleIndex++,
            level
          );
      ...
      oldState.obstacles.forEach((o) => {
        if (o.left >= OBSTACLE_WIDTH * -1) {
          ...
          if (!hasCollidedWithObstacle) {
            hasCollidedWithObstacle = this._checkColision(
              INITIAL_LEFT,
              top,
              o,
              newLeft
